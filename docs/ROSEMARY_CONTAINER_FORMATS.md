...
Research status: 2026-09-11. These are observed layouts checked against the local
SDK and package corpus, not a complete specification. The read-only
[inspector](../toolchains/mips/inspect_format.py) does not yet validate
class semantics, imported references, code architecture or runtime compatibility.
...
Scanning files by content under software/mips found and successfully inspected:
...
trailing garbage, section offsets and malformed names. Reports under
`out/rosemary-inspection/` include hashes, offsets, lengths and opaque previews.
These checks establish container readability, not all possible corruption:
...
package bytes, ROMs or save states were modified in this investigation.
...
Try has intrinsic number 50. Reproduce the comparison with:
...
bounds, unaligned records and dependency-length/padding failures. Updated corpus
and header-comparison reports remain under `out/rosemary-inspection/`.
...
Reports are in `out/rosemary-inspection/layout-comparison.json`,
`layout-summary.json` and the refreshed `corpus.json`. Per-field offsets, field
...
Re-run `compare_class_layouts.py` for the independent header comparisons.
The refreshed corpus and `member-summary.json` are in `out/rosemary-inspection/`.
...
against `Apollo/MagicCap.gnu.xh`:

```sh
python3 toolchains/mips/compare_sample_signatures.py
```

The five passing comparisons cover Draw, FillBox, ContentBox, PartColor and
...
and reference-only records. The reports are `signature-summary.json`,
`signature-comparison.json` and the refreshed `corpus.json` under
`out/rosemary-inspection/`.
...
The refreshed reports are `accessor-comparison.json`, `binding-summary.json` and
`corpus.json` in `out/rosemary-inspection/`.
...
`Debugger/Apollo/MagicCap-USA`, alongside `MagicCap-USA.image`. The new
...
These are **SDK-build addresses**, not addresses validated against the emulator's
current guest ROM. No current ROM or emulator binding was changed. Most ELF
...
The report, including source hashes, matched/unresolved methods, vector entries
and file offsets, is `out/rosemary-inspection/linked-methods.json`. Five additional
...
The disassembly is saved in `out/rosemary-inspection/frozen-loader-disassembly.txt`.
The loader evidence is from the SDK build and has not been used to patch the
current emulator. There are now 32 passing research tests.
...
`roms/MagicCap-USA.image` and `roms/MagicCap-Japan.image`: neither contains any
...
separate investigation. Do not transfer the SDK's addresses to the current ROM.
The current-ROM search results and hashes are retained in
`out/rosemary-inspection/unresolved-rom-names.json`.
...
All 30 observed function-offset tables decode, totaling 2,061 slots. Regenerate
hashed per-file reports and summary with:
...
Reports are `out/rosemary-inspection/frozen-corpus.json` and
`import-summary.json`; relevant disassembly is retained in `import-loader.txt`
and `function-offset-loader.txt`. All 38 research tests pass, including malformed
...
correctly. `scan_frozen.py` includes both totals in its summary. Tests cover the
...
Disassembly evidence is retained in `out/rosemary-inspection/code-lock-loader.txt`
and `class-method-iterator.txt`. The next boundary is parsing frozen heap object
...
matches. No ROM, save state or executable package was modified.
...
Loader disassembly is retained in `out/rosemary-inspection/heap-loader.txt` and
`object-loader.txt`; regenerate corpus reports with `scan_frozen.py` as above.
...
Its hashed report is `out/rosemary-inspection/package-methods.json`. Across the
...
All **49 research tests pass**. Import-resolution disassembly is saved in
`out/rosemary-inspection/import-resolution.txt`. Remaining work includes defined
...
Disassembly evidence is retained in `defined-component-loader.txt`,
`all-class-method-iterator.txt` and `class-array-lookup.txt` under
`out/rosemary-inspection`. The package writer and guest execution validation
remain unfinished.
...
Disassembly is saved as `object-addressing-loader.txt` and
`object-addressing-range-loader.txt` in `out/rosemary-inspection`. Regenerate
reports with `scan_frozen.py` followed by `link_package_methods.py`.
...
`scan_frozen.py` reports now include the decoded header/prefix on A0 attributes;
other attribute flags remain opaque. Disassembly is saved in
`out/rosemary-inspection/global-init-loader.txt` and `bnum-loader.txt`.
...
`scan_frozen.py` writes these totals into its summary as well as the per-file
hashed reports. The next useful validation is interpreting the supported writes
...
Report: `out/rosemary-inspection/global-init-validation.json`, with input hashes.
Seven new tests cover bounds/alignment, bounded expansion, resets/deltas, signed
...
`resolve_global_sources.py` classifies all 5,419 resolution entries without
...
Report: `out/rosemary-inspection/global-sources.json`, including package and SDK
source hashes. Five new tests cover interface boundaries, indexicals, optional
...
pass**. Saved disassembly includes `package-interchange-loader.txt`,
`package-export-callback.txt`, `package-export-stride.txt`, and
`package-export-iteration.txt` under `out/rosemary-inspection`.
...
`resolve_global_sources.py` now indexes non-local exports from **all 32 packages**,
...
`rom_interface_exports.py` now follows stored object references around the
...
Run `resolve_global_sources.py` first, then `rom_interface_exports.py`. Output
`out/rosemary-inspection/rom-interface-exports.json` includes ROM hashes, the
source-report hash, object offsets, exports and per-reference candidates. This
closes the missing-provider evidence gap for the checked US ROMs. Remaining work
...
An illustrative 40-byte payload was saved as
`out/rosemary-inspection/generated-global-init.bin`, with its hash and decoded
report in `generated-global-init.json`. It declares 20 globals bytes: four
...
**byte-for-byte identically**, including bundles. Outputs and input/output hashes
are under `out/rosemary-inspection/rebuilt/`; original packages are untouched.
...
`compare_abbreviated_classes.py` compares imported class identities against the
...
Report: `out/rosemary-inspection/abbreviated-classes.json`, with input hashes.
...
input and trailing bytes. **111 research tests pass**. Reader/writer/initializer
disassembly is retained under `out/rosemary-inspection/abbreviated-class-*.txt`.
...
Report: `out/rosemary-inspection/derived-fixed-formats.json`, including the SDK
hash, explicit base offsets, field provenance, formats and corpus comparisons.
Five tests cover scalar/reference formats, weak references, explicit mixin
...
weak**, and **23 empty bodies**. `object-list-validation.json` records source
hashes, body offsets, list formats and selector values. This verifies the list
...
`python3 toolchains/mips/validate_text_values.py` checks imported Text
objects across the existing corpus and writes a hashed source/offset report to
`out/rosemary-inspection/text-validation.json`: **862 byte-identical**, **2
...
`python3 toolchains/mips/build_empty_objects.py` now generates five object
value fragments, an imports payload, abbreviated class formats, and a manifest
in `out/rosemary-inspection/empty-package-objects/`. It explicitly transcribes
...
`python3 toolchains/mips/validate_object_names.py` follows named dictionary
objects in existing frozen packages and reproduces **all 32 dictionaries' two
backing bodies byte-for-byte**, covering **810 names**. Its report records source
hashes, original body offsets, and each name's locator selector in
`out/rosemary-inspection/object-names-validation.json`. Duplicate strings remain
...
silently clearing them. `python3 toolchains/mips/validate_clusters.py`
reconstructs **all 32 complete cluster bodies byte-for-byte**: 30
CodePackageCluster bodies and two code-free PackageCluster bodies. The reader
also validates/reconstructs all 32 PackageData fixed bodies. Source hashes,
body locations, class names, internal names, and decoded field values are in
`out/rosemary-inspection/cluster-validation.json`.
...
`python3 toolchains/mips/validate_package_metadata.py` records source
hashes and object offsets for comparisons. It checks all 32 PackageData objects,
...
See ROSEMARY_ROM_TESTING.md for the actual installation result, which is
...
opens its Scene, and displays the generated help Text. Full commands, screenshots,
states and the tested package/ROM hashes are recorded in ROSEMARY_ROM_TESTING.md
and `docs/rosemary-empty-package-validation.json`. This supersedes earlier
...
`python3 toolchains/mips/validate_native_class.py` reconstructs **50
existing classes byte-for-byte** whose layout falls within this exact subset.
Its report includes package hashes, source body offsets and selectors. Classes
with extra fields, multiple superclasses, other method kinds or different
method-list forms remain outside this constructor.

`python3 toolchains/mips/build_native_leaf.py` compiles `native_leaf.c`
...
Artifacts and hashes are under `out/rosemary-native-probe/`.
...
initialization. See ROSEMARY_ROM_TESTING.md for the exact candidate hash,
...
ROSEMARY_ROM_TESTING.md and rosemary-native-leaf-validation.json. **158 tests
...
[the 68k notes](OBJECTMAKER_FORMAT.md); `toolchains/m68k/ghidra/` has
the script.
