...
Investigated 2026-09-11 using the preserved `magicdeveloper.sit` SDK.
The build pipeline and a Linux-compatible host build path are now established.
All 17 preserved SDK sample directories build from their original sources; the
guest-validation evidence is recorded in `ROSEMARY_ROM_TESTING.md`.
...
All SDK paths below are relative to
`software/mips/sdk/extracted/MagicDeveloper/MagicDeveloper/`:
...
[toolchains/mips/probe.py](../toolchains/mips/probe.py) reproduces the
checks without a Mac, modifying neither the SDK nor the emulator:
...
- Compiler output, SDK disassembly, commands, source build scripts and object
  hashes are retained under `out/rosemary-probe/`.
...
See the latest EmptyPackage section in ROSEMARY_CONTAINER_FORMATS.md for exact
...
and regression expectations are corrected. No emulator code or ROM was changed.
...
help text. See ROSEMARY_ROM_TESTING.md for reproducible commands and hashed
validation evidence. Native MIPS method linking remains the next milestone.
...
or state patches were used. See ROSEMARY_ROM_TESTING.md for exact evidence.
...
a host-built probe (see ROSEMARY_ROM_TESTING.md, "Native-to-ROM call verified"):
...
Second probe (NativeRomCall2, ROSEMARY_ROM_TESTING.md): arguments follow o32
...
ROSEMARY_ROM_TESTING.md "C-compiled method verified"):
...
HelloWorld itself (ROSEMARY_ROM_TESTING.md) is the first package with a new
...
Guest-verified with the unmodified HelloWorld.cpp (ROSEMARY_ROM_TESTING.md):

- **Headers**: `sdk_headers.py` derives `out/rosemary-sdk-headers/` from
...
- The ROM's `DispatchObjectMethodCommon` (0x13c96e84) hands
  ...
  guest (ROSEMARY_ROM_TESTING.md, "Inherited calls").
...
first user (ROSEMARY_ROM_TESTING.md); HelloWorld still uses the older
...
- Runtime: `mcap_accessors.cpp`, a port of SeparatePackageLib.o's
...
activates and its scene opens (`docs/images/rosemary-rulessample.png`).
...
guest with the wrappers (`docs/images/rosemary-importsample.png`: the
...
1. **No empty method tables.** `SampleSpeedScroller` has no methods of its
   ...
   `GrabAbbreviatedClassTablePointers`. Found with a store watch on the
   clobbered driver word (`--watch-write`, below): the last writers before
...
2. **Header bit 0x08000000 = the extra part is a reference list.** Corpus
   ...
   (`docs/images/rosemary-rulessample.png`).
...
Diagnostic added to the MIPS core: `--watch-write <addr>` logs every store
to that word with pc, ra and instruction count (kept outside the saved CPU
struct so states stay compatible). It is what turned an interrupt-handler
crash with garbage registers into the name of the ROM routine that wrote
the garbage.
...
(`docs/images/rosemary-tictactoe.png`, `docs/rosemary-tictactoe-validation.json`).
...
- **`call intrinsic_X` in scripts.** The pool's fourth list
  ...
  with a synthetic HelloWorld variant (`out/rosemary-synth/HonkScript`): the
  button's script reaches the ROM's `Sound_Honk` through the intrinsic
  dispatcher (`docs/rosemary-honkscript-validation.json`).
