...
cookbook examples (source in `sdk/68k/samples/projects/`, compiled packages in
`sdk/68k/samples/packages/`) and checked against two real shipping packages
(`sdk/68k/fixtures/`). [`toolchains/m68k/`](../toolchains/m68k/README.md)
holds the inspector, the writer and their tests; this is what they know and,
as importantly, what they do not. For the MIPS counterpart see
...
from a better position than MIPS did -- a guest check already installs the
stock Counter package through PC Link in the MagicHat emulator, runs it, taps
its button and checks the count against a golden crop -- so a known-good
package and a guest that runs it are both in place. What was missing is being able to say
what is inside one.
...
with opcode bytes between them. A PEF pattern-data unpacker reads it properly: the data section
comes out at exactly the 61,728 bytes the header claims, and the messages read
cleanly.

Ghidra reads it as well. Its PEF loader takes the binary, applies the 478
...
17 to **589 of 956**. See also the
[CodeWarrior tool analysis](GHIDRA_CODEWARRIOR_ANALYSIS.md).
...
-- the same crop hash the Counter guest check uses for the stock package. A package this project generated installs and runs, and counts.
...
A guest check does that walk against both and requires them to agree. Twenty-seven of the twenty-nine records come out byte-identical as
...
forgetting exactly that -- install it and run the Counter guest check,
...
symbol table for the PIC-1000 ROM -- see
...
`Device/System.Equates` in the SDK (`sdk/68k/interfaces/1.0-original/Device/`)
is an MPW assembly include of eleven
...
`ROMChecksum = $DD2BE2B`, and that checksum is the word at offset 0x4c of the
Sony PIC-1000 ROM (`ROMs/68k/Sony PIC-1000/PIC-1000.rom` in the Magic Cap
preservation archive).

So it is a symbol table for one of the preserved ROMs: 10,528 routines at
exact addresses. A prologue check is what says so beyond the checksum. It
scores how often a named address begins something that looks like
the start of a routine:

| ROM | Rate |
| --- | --- |
| PIC-1000, reconstructed image | 77.7% |
...
That makes the file usable for labelling a PIC-1000 disassembly by routine
name.
...
a guest check walks each of them to its scene and requires the same pixels as the package ObjectMaker built.
...
`NoDebug/Operations.h` (for example
`sdk/68k/interfaces/1.0-original/NoDebug/Operations.h`) declares the system's operations
...
**the Counter guest check passes with the built package**: Counter compiled from its own
...
Whitehouse builds, transfers over PC Link at 115200 baud, and
...
at (240,180). A generic Hallway route instead opened the Game room, whose
animated cat clock produced different screenshots and a false failure. The
guest check therefore uses Whitehouse's Downtown route and a
...
The title/application crop has SHA-256
`431a7de76f2969ea961390876d51c5cf8095be8c09494c0a5a079af5c9cdbb58` after
excluding the animated hallway clock.
The check also covers reset with zero counts and a save/restore cycle.
...
The full code-bearing matrix passes for all eight examples. The generic route reaches the Hallway
