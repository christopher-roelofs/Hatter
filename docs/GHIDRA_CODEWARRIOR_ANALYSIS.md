...
a claim that the proprietary tools have been decompiled. Hatter's builders do
not run these tools; the findings explain the formats the builders write. The
package-level results are in the [68k container notes](OBJECTMAKER_FORMAT.md)
and the [Rosemary container notes](ROSEMARY_CONTAINER_FORMATS.md).
...
The CW8 tools come from `CW8_Gold_Tools_199601.cdr` (in the Magic Cap
preservation archive under `Software/68k/CodeWarrior/`). The important
interface roots are `Magic Cap Support/Interfaces/Only 1.0`,
...
PowerPC big-endian PEF, using headless analysis with a 120-second
per-file timeout. ObjectMaker is `MagicDeveloper/Tools/ObjectMaker` inside the
CW7 `CodeWarrior Magic/MPW Installer` (from `CW7_Gold_Tools_199509.cdr`); the
other five are in `MagicDeveloper/Tools/` of the Magic Developer SDK
(`Software/MIPS/SDK/magicdeveloper.sit` in the preservation archive). No
Ghidra project is kept; the analysis is reproducible from those binaries.

PEF code reaches its data through r2, which CFM sets from the main transition
vector rather than from anything in the instruction stream. Setting r2 in the
register context and re-analysing is what makes the string references
resolve. The [68k notes](OBJECTMAKER_FORMAT.md#reading-the-tool) give the
ObjectMaker value and the
[Rosemary notes](ROSEMARY_CONTAINER_FORMATS.md#fixedformat-and-the-initialization-script-decoded-2026-09-20)
the FrozenDump one.
...
The resource forks remain valuable. Each tool has a companion `.rsrc` file
when the archive is extracted with its resource forks preserved. Resource inspection should be a separate
